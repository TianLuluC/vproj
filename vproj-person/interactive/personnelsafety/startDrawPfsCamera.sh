...
echo "====================  配置摄像头参数 ===================="
# 如果脚本运行没有传参，则使用默认的硬编码参数--此处为传进shell脚本的传参顺序
cameraAddress=${2:-"rtsp://admin:hik12345@10.32.132.20:554/h264/ch1/main/av_stream"}
cameraId=${1:-"1314"}
cameraLocation=${3:-"12"}
...
docker exec -d "$CONTAINER_ID" sh -c "nohup python3 regoin_stream.py $RUN_ARGS >/dev/null 2>log &"
echo "Start success!"
